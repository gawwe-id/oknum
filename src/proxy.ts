...
export default clerkMiddleware(async (auth, req) => {
  ...
  if (!userId) {
    // Check if trying to access any protected route
    if (isAdminRoute(req) || isExpertRoute(req) || isStudentRoute(req)) {
      // Preserve the original URL as redirect_url parameter
